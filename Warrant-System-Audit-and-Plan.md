...
- [ ] **Galveston failed — corrected diagnosis, 2026-08-23 (later same day)**: the original "probably this sandbox's IP" theory is disproven. Re-tested from a completely different network via the real Chrome browser and got the identical `ERR_CONNECTION_RESET` on `p2c.galvestoncountytx.gov`. Also found that the county's own current website (`www.galvestoncountytx.gov`, which itself loads fine) now links to a *different* jail-roster URL, `p2c.co.galveston.tx.us` — tested that one too and it fails identically. Both hostnames show the same signature: port 80 works fine (BigIP load balancer answers instantly with a clean `302 → https`), but port 443 completes a full TLS handshake, accepts the request, then resets with zero bytes back — on the bare root and on `/jailinmates.aspx` alike, on both hostnames. That pattern (front-end LB alive, backend app never answers) looks like a real, current outage of the P2C jail-roster backend itself, not IP-based blocking or bot detection — a WAF/anti-bot system would return an actual HTTP error or challenge page (as Brazoria's does), not a bare reset with nothing behind it. **Likely transient — retest in a day or so before assuming it needs a fix.** If it's still down after that, this is a normal "your public roster site appears to be down" courtesy report to the Sheriff's office IT, not a bot-access request like Brazoria's.
...
- [x] **Fixed 2026-08-23 (later same day)** — scope was actually bigger than "Harris-only": `Case`/`CaseHarris` was bound to `simple_harris`, a small (51-doc), separately-running legacy feed (`source_system: 'harris_district_clerk'`, court/bond civil filings — not jail booking data), a completely distinct `_id` space from `v2_harris_reports` (488 docs), which is what actually powers the Cases list. Verified live before fixing: `PATCH /cases/:id/tags` on a real, currently-visible Harris case sourced from `v2_harris_reports` returned a clean `404`; the same route against a real `simple_harris`-sourced case returned `200`. So the write routes didn't actually work for "Harris County" — only for that ~51-doc legacy subset, 404ing honestly for virtually every case in the dashboard, across every county including Harris.
  - **Chose option B** (unified, app-owned `CrmOverlay` collection, `apps/dashboard/server/src/models/CrmOverlay.js`, keyed by the case's own `_id` — the same identity already used by `CaseAudit`/`Message`/`CaseEnrichment`, none of which needed to change) over extending `Case` models to bind directly to `v2_*` collections, specifically because `v2_*` is owned/upserted by `services/warrantdb-pipeline` on every scrape cycle — CRM fields living in those documents risked a re-scrape silently clobbering CRM state.
  - New shared `apps/dashboard/server/src/routes/utils/caseAccess.js` (existence + department/county scoping, extracted and reused by `cases.js`/`documents.js`/`checkins.js`, replacing three divergent local copies — `documents.js`'s was missing `county` from its scope fields, `checkins.js`'s only checked `county`, both silently narrower than the canonical 5-field list).
  - Rewired: the 3 CRM write routes, `GET /cases/:id`, `GET /cases` (list), all 4 `documents.js` attachment routes, and `checkins.js`'s client-picker typeahead (previously queried `Case`/`CaseJefferson` directly for a cross-case search — wrong target regardless of the collection-binding fix, now uses the same `v2_*` union-aggregation pattern `GET /cases` uses).
  - One-time migration script (`apps/dashboard/server/scripts/migrate_crm_to_overlay.js`, dry-run by default) written to backfill any real historical CRM data from the legacy `simple_harris`/`simple_jefferson` subset — **not yet run against production Atlas**, that's on you when ready (purely additive, safe to run anytime before or after this code is live).
  - **Verified live against the real running system** (not just unit tests): the exact same `PATCH /cases/:id/tags` request that 404'd before the fix now returns `200` and persists, on a real currently-visible Harris case and a real Fort Bend case (which never worked under any prior framing, including the old "Harris-only" one). Confirmed idempotency preserved (`PATCH /stage` twice with the same value writes exactly one audit record, verified by querying `CaseAudit` directly). Confirmed backward compatibility with the legacy `simple_harris` subset. Confirmed `documents.js`'s attachment routes and `checkins.js`'s client typeahead (now returning 40 real cases instead of a near-empty legacy-only list) all work end-to-end. Full existing test suite (`server/tests/cases.crm_enrichment.test.js`, updated for the new `CrmOverlay`-based flow, plus 3 other unrelated suites) passes.
  - **Not done, deliberately**: the migration script hasn't been run against production; a scoped (non-global, department-restricted) test user's access wasn't verified against the newly-enforced scoping fix on `GET /cases/:id` (previously unscoped — now correctly gated, worth a manual QA pass before this is fully considered done).
...
- [x] **Done 2026-08-23**: Migrated the dashboard's messaging system from Twilio to Telnyx — straight cutover, no dual-provider period. New `server/src/lib/messaging/telnyx.js` (`sendTelnyxMessage`, real ed25519 webhook signature verification via `tweetnacl`), `server/src/services/messaging.js` (`applyTelnyxStatusUpdate` replaces `applyTwilioStatusUpdate`; Telnyx never resolves delivery synchronously, only via webhook, unlike Twilio), `server/src/routes/messages.js` (`telnyxWebhooks` replaces `twilioWebhooks`), raw-body-preserving JSON parser mounted ahead of the global body parser in `index.js` (mirrors the existing Stripe webhook pattern). Removed `twilio.js` and the `twilio` npm dependency entirely; `Message.provider` is a free-text field so historical Twilio-era rows still render correctly. Verified: server boots clean in the local sandbox, new `GET /api/admin/telnyx/status` route responds correctly (401 unauthenticated, 200 as SuperUser), confirmed live in-browser on the Admin page. **Not yet done**: obtaining a `TELNYX_MESSAGING_FROM_NUMBER`/`TELNYX_MESSAGING_PROFILE_ID` and `TELNYX_PUBLIC_KEY` for the dashboard, confirming the real ed25519 signed-payload delimiter against a captured webhook, and configuring the Messaging Profile's webhook URLs to point at `/api/messages/telnyx/status` and `/api/messages/telnyx/inbound`. **Correction, 2026-08-23 (later same day)**: an earlier note here claimed ai-agent's `.env` already had `TELNYX_API_KEY` set — that was wrong. It had `TELNYX_TOOL_TOKEN`/`TELNYX_WEBHOOK_SECRET` only (inbound auth for its own webhooks, not an outbound Telnyx account API key). The user has since provided a real `TELNYX_API_KEY` (added to `services/ai-agent/.env`, gitignored) for inspecting the live Telnyx Assistant — see the new entry below. Whether this same account-level key can be reused as the dashboard's sender credential is still unconfirmed.
- [x] **Done 2026-08-23 (later same day)**: Inspected the live Telnyx AI Assistant ("Tiffany", ASAP Bail Bonds) via Telnyx's real Assistants API (`GET /v2/ai/assistants`), using a real account API key the user provided for this purpose. Findings: all 13 real tools the instructions reference are correctly registered with matching webhook URLs/auth/schemas (an initial "tools: []" reading was just the list-endpoint omitting detail, not an actual gap). Confirmed every tool webhook points at `ai-agent-warrant.onrender.com` — the original standalone repo's Render deployment, not a new deployment from this monorepo; checked whether that matters by comparing against the standalone repo's git history (last commit 2026-05-01, well before the 2026-08-22 merge, zero commits since) — the monorepo copy is very likely functionally in sync with what's actually live. Found and removed one genuinely dead tool, `Update-Inmate` (`/telnyx/update_inmate`) — no backing route exists anywhere, monorepo or original standalone repo; it was a Telnyx-portal-only placeholder the instructions themselves said never to call except by direct supervisor request. Removed it (and its one instruction-text mention) via a targeted script (dry-run previewed first, then applied) — verified via re-fetch, nothing else on the assistant touched. **Deliberately not changed, flagged for the user's own judgment, not decided unilaterally**: the model (`Qwen/Qwen3-235B-A22B`, an unusual choice for a production voice assistant vs. more common GPT-4o/Claude-class defaults) and the fact the assistant's last version bump was 2025-11-19 (~9 months stale) — no evidence either is actually causing a problem on real calls, so no changes recommended without a concrete signal (a bad call, a cost concern, a missing capability) to act on.
- [x] **Done 2026-08-23**: Built a real Telnyx integrations panel in `Admin.jsx` (new `TelnyxStatusPanel.jsx` + `hooks/telnyxStatus.js`, following the `ScraperOpsPanel`/`adminIngestion.js` React Query + manual-refresh convention) — replaces the old "Integration management coming soon" placeholder.
- [ ] Finish check-ins reminders/GPS queue (documented as outstanding in `final-feature-readiness.md`).
- [x] **Done 2026-08-23**: Display scraped mugshots (Cases list thumbnail + Case detail) wherever the underlying record actually has one. Only Galveston (`media[].rel:'mugshot'`) and Jefferson (`mugshot_url`) scrapers capture a photo at all; both now flow through `GET /cases` and `GET /cases/:id` via a shared `extractMugshotUrl()` helper in `routes/cases.js`, filtering out the jail vendor's own "missing-image-profile" placeholder sentinel. Verified against real data: 6 real Jefferson photos (public S3/CloudFront-backed, no hotlink protection — confirmed via `curl -I`) render correctly in both the Cases list and Case detail page; rows without a photo show nothing, no broken-image icon. Galveston currently has zero populated `media` records in the sandbox (consistent with its scraper being blocked) — its URL format is unverified for hotlinking and should be spot-checked once real data exists again. Hit and fixed one real bug along the way: Tailwind's `img{max-width:100%}` preflight rule collapsed the thumbnail to 2px wide inside the table's auto-layout column — fixed by wrapping the `<img>` in a fixed-size `div` (not itself subject to the preflight rule) in both `Cases.jsx` and `CaseDetail.jsx`.
- [ ] **Deferred, not built** — original blocker resolved 2026-08-23 (later same day): a manual "add photo" upload for a case once it becomes an actual client was scoped out because it would inherit the Harris-only case-write bug. That bug is now fixed (see Phase 3.5's CRM overlay entry) — writes work for all 6 counties, so this is no longer blocked on that. Still not built: pre-decided for whenever this ships, storage should be **S3** (mirroring `services/ai-agent`'s existing AWS usage), not the local-disk pattern the existing case-document upload feature uses today — that pattern does not survive a Render redeploy.
- [x] **Done 2026-08-23/24 — real production outage found and fixed**: the user reported the live bot "rings once and hangs up." Diagnosed via Telnyx's real `/v2/ai/conversations` API (not guesswork): every call since **2026-07-29** failed with `"The assistant could not generate the greeting audio"` — the ElevenLabs voice provider, not the backend. Ruled out key validity/permissions/quota/voice-access one by one via direct ElevenLabs API tests (two separate keys, both fully verified working) before concluding the failure was inside Telnyx's own ElevenLabs integration, not diagnosable further from outside their platform. **Fix applied**: switched the assistant's voice from `ElevenLabs.eleven_multilingual_v2.6aDn1KB0hjpdcocrUkmq` to the Telnyx-native `Telnyx.Ultra.86600680-b836-41e1-9916-8475728dcc14` ("Tiffany - Dynamic Presence" — same persona name, zero external dependency), applied via Telnyx's Assistants API. Also found and removed the dead `Update-Inmate` tool during this same investigation (see above).
- [x] **Done 2026-08-24 — second, independent, more severe bug found and fixed**: user asked to verify the agent can actually find real inmates. Live production test of `find_person` returned `found:false` for a real, currently-booked Harris inmate. Root cause (confirmed against real Atlas data, not assumed): `find_person`/`get_bail_status` only ever searched `simple_*` collections, which had **stopped receiving new bookings 8-12+ months ago** in every county they cover (`simple_harris` frozen since 2025-12-14; `simple_brazoria`/`simple_galveston`/`simple_fortbend` since September 2025), and never covered Jefferson/Wharton at all. Confirmed 20/20 of the most recent real Harris bookings were completely absent from `simple_harris`; the fallback (`persons`/`custody_events`) doesn't cover for it either — `custody_events` has zero documents in production. This is the identical `simple_*` vs `v2_*` architectural bug already fixed in the dashboard, independently present here. **Fix**: added `v2_*` as a second search pass across all 6 counties (`services/ai-agent/app/telnyx_tools.py`'s `_find_in_simple`/new `_list_v2_cols`), including correct county-scoping for the shared `v2_lookup_results` collection (fortbend/jefferson/brazoria) — verified no cross-county leakage. **Also found and fixed a second, pre-existing bug while testing this**: `_list_simple_cols`'s fallback loop did `if col and col not in ordered` — this pymongo version raises `NotImplementedError` on `bool(Collection)` (deliberately disallowed; requires `is not None`). This predates today's work entirely — any Jefferson/Wharton-scoped call, or any call with *no* county hint at all, has likely been hard-crashing with a 500 for a long time, independent of the stale-data issue. Fixed with a one-line change. Both fixes applied to **both** `services/ai-agent` (this monorepo) and the standalone `AI_Agent_Warrant` repo (confirmed via git remote that `ai-agent-warrant.onrender.com` — the number real callers actually reach — deploys from the standalone repo, not this monorepo) and pushed live; verified end-to-end against the real production webhook across Harris/Wharton/Fort Bend and the no-county-hint case. **Not fixed, separate and already known**: `v2_*` itself is also stale (frozen ~2026-05-01, ~3.5 months) — this fix recovers real data from roughly Sept 2025–May 2026 the agent couldn't see before, but bookings from the last ~3.5 months still won't be found until the dormant production scraper pipeline (Phase 3) resumes.
