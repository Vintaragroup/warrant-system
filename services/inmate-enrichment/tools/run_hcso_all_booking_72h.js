...
(async () => {
  ...
  const rawDocs = await col.find({}, { projection: { dob: 1, spn:1, subject_id:1, subjectId:1, bond:1, bond_amount:1, ...Object.fromEntries(bookingFields.map(k=>[k,1])) } })
    .sort({ _id: -1 })
    .limit(20000)
    .toArray();
  const filtered = rawDocs.filter(d => {
    ...
    return b && b >= cutoff && !hasDob && bondVal >= MIN_BOND;
  });
  ...
  console.log(`Total recent by booking<=72h missing dob (bond >= $${MIN_BOND}):`, subjectIds.length);
  ...
  // Send in chunks of 50 to avoid huge payloads
  const chunk = (arr, n) => arr.length ? [arr.slice(0, n), ...chunk(arr.slice(n), n)] : [];
  const chunks = chunk(subjectIds, 50);
  const allJobIds = [];
  for (const c of chunks) {
    ...
    allJobIds.push(...(data.jobIds || []));
  }
  console.log('Queued total jobs:', allJobIds.length, 'suffix:', jobSuffix);
