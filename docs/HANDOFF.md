...
|---|---|---|---|
| `warrantdb-api` (dashboard backend) | Render (Docker, Starter) | 🟢 Live, healthy, real login verified | `warrant-system` / `main` — **done** |
| Dashboard frontend (`warrant-system-dashboard`) | Vercel | 🟢 **Live** — `apps/dashboard/vercel.json` supplies the build config + SPA rewrite. Real login verified end-to-end. | `warrant-system` / `main` — **done** |
| `warrantdb-api-sessions` (Redis) | Render (free Key Value) | 🟢 Live | n/a — provisioned 2026-08-24, see note below |
...
**What's actually verified working end-to-end** (real browser/API tests, not just "it deployed"): dashboard login on the new Vercel+Render stack (real session cookie, real user payload), case list/detail with real scraped data, CRM writes (tags/stage/notes) across all counties, document/enrichment routes, the Telnyx AI agent's `find_person`/`get_bail_status`/`transfer_plan` tools against real data.

**What's not yet verified on the *new* deployment specifically**: a real CRM write against `warrantdb-api`'s new deployment post-login (backend itself was verified pre-frontend; login is now confirmed too, but a full click-through — case list, a CRM edit — hasn't happened since); the pipeline crons haven't run at all yet from `warrant-system` (still pending deploy).
...
12. ~~Building `apps/dashboard` from the monorepo root was unreliable~~ — **fixed 2026-08-24**. `apps/dashboard` needed `vite@7.x` while `vitest` (also a dashboard dependency) needed `vite@5.x` internally; across the full monorepo tree, npm's hoisting for that conflict was non-deterministic between installs — sometimes landing `@vitejs/plugin-react` and `vite` in different, mutually-unreachable locations, breaking both local `npm run dashboard:build` and, as it turned out, **Vercel's actual build too** (Vercel installs from the repo root, not an isolated `apps/dashboard` directory, so it hit the exact same conflict). Fixed by bumping `vitest` `^1.6.0` → `^3.2.4` (supports `vite` 6/7/8, eliminating the conflicting version requirement entirely) — confirmed via a full simulation of Vercel's real build (fresh clone, root-level `npm install`, then build) that this converges cleanly and deterministically now, plus a real local rebuild and the full test suite passing. `apps/dashboard/vitest.config.ts` logs one new deprecation warning (`environmentMatchGlobs` → `test.projects`) — not blocking, worth migrating at some point.
13. **Forgot-password doesn't deliver email.** `POST /api/auth/forgot-password` works correctly (generates a real reset token, always returns 202 regardless of whether the account exists, by design) but no SMTP credentials (`SMTP_HOST`/`SMTP_USER`/`SMTP_PASS`/`EMAIL_FROM`) are set on the live backend, so `apps/dashboard/server/src/lib/mailer.js` silently skips sending — the user sees a normal "check your email" response and nothing ever arrives, with no error surfaced anywhere. Needs real SMTP credentials from any provider (SendGrid, Postmark, SES, etc.) set on `warrantdb-api`.
14. ~~Login hung indefinitely (timed out with no response)~~ — **fixed 2026-08-24**. Sessions are Redis-backed (`server/src/lib/session.js`, `connect-redis`) with no fallback store, but no Redis instance existed for the newly-rebuilt `warrantdb-api` — `REDIS_URL` defaulted to `redis://localhost:6379`, which doesn't exist on Render, and `ioredis`'s retry strategy never gives up, so every request touching a session (including login) hung forever instead of failing fast. Fixed by provisioning a free Render Key Value (Redis) instance (`warrantdb-api-sessions`) and setting `REDIS_URL` to its internal connection string. `infra/render/dashboard.render.yaml` now documents this requirement explicitly so it isn't missed on a future rebuild.
