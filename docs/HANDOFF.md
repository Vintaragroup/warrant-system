...
12. **Building `apps/dashboard` from the monorepo root is unreliable.** `npm install` at the repo root, followed by `npm run dashboard:build` (or `dashboard:dev`), sometimes fails with `Cannot find package 'vite' imported from .../node_modules/@vitejs/plugin-react/dist/index.js`. Root cause: `apps/dashboard` needs `vite@7.x` while `vitest` (also a dashboard dependency) needs `vite@5.x` internally — across the full ~972-package monorepo tree, npm's hoisting decision for this conflict is genuinely non-deterministic between installs (reproduced repeatedly on a clean clone with identical inputs). It does **not** affect Vercel (which builds `apps/dashboard` in isolation, where this conflict doesn't arise the same way) or `npm test`. If it happens to you: `cd apps/dashboard && rm -rf node_modules && npm install` installs it in isolation and reliably works. The real fix would be upgrading `vitest` (currently 1.6.1) to a version supporting `vite` 7, removing the conflicting version requirement entirely — untested, not done here.
