...
**Compatibility aliases added to V2 (2026-04-27):**
`booked_at` (= `booking_date`), `event_date` (= `booking_date`),
`county_display` (`"Galveston"`), `county_normalized` (`"galveston"`),
...
3. ✅ Compatibility aliases (`booked_at`, `event_date`, `county_display`, `county_normalized`,
   `charge_description`) already added to `normalize_event()` — dashboard reads will not
   require field-name changes after read-path switchover.
4. Execute one-time migration: rewrite all legacy `galveston_events` docs to use
   `{county: "galveston", booking_number: <value>}` upsert key and lowercase `county`.
5. Drop old `source_id`-based unique index; apply new `{county, booking_number}` unique index.
6. Cut over v2 as primary writer; retire `galveston_p2c_fast.py`.
